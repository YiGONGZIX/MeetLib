...
class MeetService extends BaseProjectService {
	...
	async checkMeetLimitSet(userId, meet, timeMark, nowCnt) {
		...
		if (maxCnt > 1) {
			if (cnt >= maxCnt) {
				this.AppError(`您本时段已经预约${cnt}人次，每时段最多可预约${maxCnt}人次，不能继续预约`);
			}
		}


		if ((cnt + nowCnt) > maxCnt) {
			this.AppError(`您本时段已经预约${cnt}人次，最多还可以预约${maxCnt - cnt}人次，请修改预约信息`);
		}
	}
	...
	async getMyJoinDetail(userId, joinId) {
		...
		let fields = 'JOIN_COMPLETE_END_TIME,JOIN_IS_CHECKIN,JOIN_REASON,JOIN_MEET_ID,JOIN_MEET_TITLE,JOIN_MEET_DAY,JOIN_MEET_TIME_START,JOIN_MEET_TIME_END,JOIN_STATUS,JOIN_ADD_TIME,JOIN_CODE,JOIN_FORMS';
